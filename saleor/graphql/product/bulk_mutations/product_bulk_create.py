...
from ....product.models import ProductType  # Make sure it's imported at the top


def get_results(instances_data_with_errors_list, reject_everything=False):
    results = []
    for data in instances_data_with_errors_list:
        product = data.get("instance")
        errors = data.get("errors", [])
        results.append(ProductBulkResult(product=product, errors=errors))
    return results
...
class ProductBulkCreateInput(ProductCreateInput):
    ...
    product_type = graphene.String(
        description="Name of the product's type.",
        required=True,
    )
    ...
    # product_reference = graphene.String()
    product_name = graphene.String()
    channel_list = graphene.String()
    gender = graphene.String()
    metal = graphene.String()
    shape = graphene.String()
    styles = graphene.String()
    thumbnail = graphene.String()
    thumbnail_hovered = graphene.String()
...
class ProductBulkCreate(BaseMutation):
    ...
    @classmethod
    def generate_unique_slug(cls, slugable_value, new_slugs): # this function is used to generate a unique slug for a product
    ...
    ): # Validates and cleans basic product fields (e.g., weight, description, slug, SEO fields).
    ...
    @classmethod
    def add_indexes_to_errors(cls, index, error, index_error_map, path_prefix=None): # Adds error messages to the error map for a specific product index, formatting error paths.
    ...
    @classmethod
    def clean_attributes(cls, cleaned_input, product_index, index_error_map): # Validates and cleans product attributes, handling errors if attribute assignment fails.
    ...
    ): #Checks for invalid or duplicate channel IDs in the product's channel listings.
    ...
    ): # Sets the available_for_purchase_at field in channel data based on input flags.
    ...
    @staticmethod
    def set_published_at(channel_data): # Sets the published_at field in channel data if the product is marked as published.
    ...
    def clean_product_channel_listings(
        ...
        used_channels_map,
        index_error_map,
    ): # Validates and prepares channel listing data for a product, including error handling.
    ...
    @classmethod
    def clean_media(cls, media_inputs, product_index, index_error_map): # Validates and prepares media (images, URLs) for a product, handling errors for missing or invalid data.
        media_to_create = []
    ...
    def clean_variants(
        ...
        product_index,
        index_error_map,
    ): # Validates and prepares product variants, collecting errors for each variant.
    ...
    ): # Orchestrates the cleaning and validation of all product input fields (base fields, attributes, media, channel listings, variants).
        used_channels_map: dict = {}
        base_fields_errors_count = 0

        try:
            # Create a copy of the input data to preserve all fields
            cleaned_input = data.copy()

            # Inject product_type object by name lookup
            product_type_name_or_id = cleaned_input.get("product_type")

            try:
                product_type_obj = ProductType.objects.get(name__iexact=product_type_name_or_id)
                cleaned_input["product_type"] = product_type_obj
            except ProductType.DoesNotExist:
                index_error_map[product_index].append(
                    ProductBulkCreateError(
                        path="productType",
                        message=f"Product type '{product_type_name_or_id}' does not exist.",
                        code=ProductBulkCreateErrorCode.NOT_FOUND.value,
                    )
                )
                return None

            # Set name from product_name if not already set
            if not cleaned_input.get("name") and cleaned_input.get("product_name"):
                cleaned_input["name"] = cleaned_input["product_name"]

            # Generate slug from name if not provided
            if not cleaned_input.get("slug") and cleaned_input.get("name"):
    ...
    @classmethod
    def clean_products(cls, info, products_data, index_error_map): # Cleans and validates a list of product inputs, returning a map of cleaned data.
        ...
        # print('warehouse_global_id_to_instance_map :', warehouse_global_id_to_instance_map.values())
        channel_global_id_to_instance_map = {
            graphene.Node.to_global_id("Channel", channel.id): channel
            for channel in models.Channel.objects.all()
        }
        # print('channel_global_id_to_instance_map :', channel_global_id_to_instance_map.values())
        ...
        for product_index, product_data in enumerate(products_data):
            ...
            print('cleaned_inputs_map :', cleaned_inputs_map)
        return cleaned_inputs_map

    @classmethod
    def create_products(cls, info, cleaned_inputs_map, index_error_map): # Constructs product instances from cleaned input data, prepares related data (media, variants, channel listings), and collects errors.
    ...
    @classmethod
    def create_variants(cls, info, product, variants_inputs, index, index_error_map): # Constructs product variant instances from cleaned input data, prepares related data, and collects errors.
        ...
        for variant_data in variants_inputs:
            if variant_data:
                try:
                    ...
                    if attributes:
                        # Convert attributes to the expected format if needed
                        if not isinstance(attributes, list):
                            attributes = [attributes]
                        # Ensure each attribute is a tuple of (attribute, values)
                        ...
                        for attr in attributes:
                            if isinstance(attr, dict):
                                ...
                                if attr_id and values:
                                    formatted_attributes.append((attr_id, values))
                            else:
    ...
    @classmethod
    def save(cls, info, product_data_with_errors_list): # Performs bulk creation of products, media, channel listings, and attributes. Also prepares data for variants.
    ...
    @classmethod
    def _save_m2m(cls, _info, instances_data): # Handles saving many-to-many relationships, such as assigning products to collections.
        product_collections = []
    ...
    def prepare_products_channel_listings(
        cls, product_data, listings_input, listings_to_create, updated_channels
    ):
    ...
    @classmethod
    def save_variants(cls, info, variants_input_data): #Delegates saving of product variants to the appropriate bulk create method.

        return ProductVariantBulkCreate.save_variants(info, variants_input_data, None)

    @classmethod
    def prepare_media(cls, info, product, media_inputs, media_to_create): # Prepares media objects (images, external URLs) for bulk creation, including file validation and downloading.
    ...
    @classmethod
    def post_save_actions(cls, info, products, variants, channels): # Triggers post-save actions such as webhooks, plugin events, and promotion rule updates after products and variants are created.
        manager = get_plugin_manager_promise(info.context).get()
        product_ids = []
        webhooks = get_webhooks_for_event(WebhookEventAsyncType.PRODUCT_CREATED)
        for product in products:
            # Create ChannelContext wrapper for the product
            product_context = ChannelContext(node=product, channel_slug=None)
            # Ensure the product is properly wrapped for GraphQL context
            product_context.node = product
            cls.call_event(manager.product_created, product_context, webhooks=webhooks)
            product_ids.append(product.id)

        webhooks = get_webhooks_for_event(WebhookEventAsyncType.PRODUCT_VARIANT_CREATED)
        for variant in variants:
            # Create ChannelContext wrapper for the variant
            variant_context = ChannelContext(node=variant, channel_slug=None)
            # Ensure the variant is properly wrapped for GraphQL context
            variant_context.node = variant
            cls.call_event(manager.product_variant_created, variant_context, webhooks=webhooks)
    ...
    def perform_mutation(cls, root, info, **data): # The main entry point for the mutation. Orchestrates validation, creation, error handling, saving, and post-save actions for the bulk product creation process.
        print('data :', data)
        index_error_map: dict = defaultdict(list)
        # print("index_error_map 0 : ", index_error_map)
        ...
        # print('cleaned_inputs_map :', cleaned_inputs_map)
        for idx, errors in index_error_map.items():
            for error in errors:
                print(f"Product 123 {idx}: path={error.path}, message={error.message}, code={error.code}")


        # print('index_error_map 1 :', index_error_map)
        instances_data_with_errors_list = cls.create_products(
            info, cleaned_inputs_map, index_error_map
        )
        # print('index_error_map 2 :', index_error_map)
        # for idx, errors in index_error_map.items():
        #     for error in errors:
        #         print(f"Product {idx}: path={error.path}, message={error.message}, code={error.code}")
        # print('instances_data_with_errors_list :', index_error_map)
        # check error policy
        if any(index_error_map.values()):
            if error_policy == ErrorPolicyEnum.REJECT_EVERYTHING.value:
                # print('index_error_map  3 :', error_policy)
        ...
        # print('updated_channels :', updated_channels)
        # save m2m fields
