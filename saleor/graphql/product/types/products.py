...
class ProductVariant(ChannelContextType[models.ProductVariant]):
    ...
    # variant_reference = graphene.String()
    variant_reference = graphene.String(
        description="Reference string for this product variant.",
        required=False,
    )

    @staticmethod
    def resolve_variant_reference(root: ChannelContext[models.ProductVariant], info):
        # Adjust this if your model field is named differently
        return getattr(root.node, "variant_reference", None)
    class Meta:
...
class Product(ChannelContextType[models.Product]):
    ...
    wishlist_id = graphene.ID(
        description="The ID of the wishlist item for the current user and this product, or null if not in wishlist.",
        required=False,
    )
    # product_reference = graphene.String(
    #     description="Reference string for this product.",
    #     required=False,
    # )

    @staticmethod
    def resolve_product_reference(root: ChannelContext[models.Product], info):
        # Adjust this if your model field is named differently
        return getattr(root.node, "product_reference", None)
    ...
    @staticmethod
    def resolve_wishlist_id(root: ChannelContext[models.Product], info):
        user = info.context.user
        if not user or not user.is_authenticated:
            return None
        from saleor.wishlist.models import Wishlist
        wishlist_item = Wishlist.objects.filter(customer=user, product_id=root.node.id).first()
        if wishlist_item:
            # Return the global ID for GraphQL
            from graphene import Node
            return Node.to_global_id("Wishlist", wishlist_item.id)
        return None
