...
from graphql import GraphQLBackend, GraphQLDocument, GraphQLSchema, get_default_backend
...
class GraphQLView(FileUploadGraphQLView):
    # Unified view for both regular and file upload GraphQL requests
    ...
    def dispatch(self, request, *args, **kwargs):
        if request.method == "GET":
            if getattr(settings, "PLAYGROUND_ENABLED", False):
                return self.render_playground(request)
            return HttpResponseNotAllowed(["OPTIONS", "POST"])
        if request.method == "POST":
            # Use graphene_file_upload's place_files_in_operations for multipart
            if (request.content_type or "").startswith("multipart/form-data"):
                operations = json.loads(request.POST.get("operations", "{}"))
                files_map = json.loads(request.POST.get("map", "{}"))
                files = request.FILES
                request._body = place_files_in_operations(operations, files_map, files)
    ...
    def get_graphql_params(request: HttpRequest, data: dict):
        ...
        if request.content_type == "multipart/form-data" and not query:
            if hasattr(data, "get") and "operations" in data:
    ...
    def get_context(self, request):
        # Ensure compatibility with ASGI request
