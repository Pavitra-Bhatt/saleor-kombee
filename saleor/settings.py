...
default_db = dj_database_url.parse(
    os.getenv("DATABASE_URL", "postgresql://neondb_owner:npg_duhKJB6AIC0o@ep-curly-tree-a1i1u69o-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require"),
    conn_max_age=int(os.getenv("DB_CONN_MAX_AGE", 600)),
)

# If USE_REPLICA is set to "true", configure a separate replica DB, otherwise point replica to default
if os.getenv("USE_REPLICA", "false").lower() == "true":
    replica_db = dj_database_url.parse(
        os.getenv("REPLICA_DATABASE_URL", "postgres://saleor:saleor@localhost:5432/saleor"),
        conn_max_age=int(os.getenv("DB_CONN_MAX_AGE", 600)),
    )
else:
    replica_db = default_db
...
JWT_TTL_ACCESS = datetime.timedelta(
    seconds=parse(os.environ.get("JWT_TTL_ACCESS", "8 hour"))
)
JWT_TTL_APP_ACCESS = datetime.timedelta(
    seconds=parse(os.environ.get("JWT_TTL_APP_ACCESS", "8 hour"))
)
JWT_TTL_REFRESH = datetime.timedelta(
    seconds=parse(os.environ.get("JWT_TTL_REFRESH", "60 days"))
)
